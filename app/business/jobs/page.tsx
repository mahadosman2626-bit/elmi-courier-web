        ...
        <div className="flex gap-2 flex-wrap mb-6">
          {statuses.map((s) => (
            <button key={s} onClick={() => setFilter(s)}
              ...
              style={{ background: filter === s ? 'var(--primary)' : 'var(--background)', color: filter === s ? 'white' : 'var(--text-secondary)', border: '1.5px solid', borderColor: filter === s ? 'var(--primary)' : 'var(--border)' }}>
            ...
            {filtered.map((job) => (
              <button key={job.id} onClick={() => router.push(`/business/jobs/${job.id}`)}
                ...
                <div className="flex items-center gap-4 text-xs mt-1" style={{ color: 'var(--text-secondary)' }}>
                  ...
                  {job.driver && <span>Driver: {job.driver.name}</span>}
